...
ベクトルを \textbf{\textit{Bold Italic}} 体 $\symbfit{u}$ にしたい場合は \verb|\symbfit{u}| というコマンドを使用してください．
このテンプレートでは \href{https://ctan.org/pkg/bm}{\texttt{bm}} パッケージを使用していません．

\begin{table}[t]
    \centering
    \caption{How to typeset Greek letters.}
    \label{tb:greek}
...
\subsubsection{\texttt{tabular} 環境を用いた表}
\label{sssec:tabular}
...
\subsubsection{\texttt{tblr} 環境を用いた表}
\label{sssec:tblr}
...
\section{定理環境・かっこいい枠}
\label{sec:theorem-box}

\subsection{定理環境}
\label{ssec:theorem}

数学や物理の定理や法則を示すための定理環境は以下のものが使えます．
...
定理~\ref{thm:example} に関して，私は真に驚くべき証明を見つけたが，この余白はそれを書くには狭すぎる．
...
定理環境としても使えるかっこいい枠を \href{https://ctan.org/pkg/tcolorbox}{\texttt{tcolorbox}} パッケージで作成できます．
標準の色はグレーです．
毎回色を変えるのは面倒なので，このレポートテンプレートでは枠の色に青・赤・緑の 3 色を用意しました．

\begin{tcolorbox}[title={グレーのカラーボックス（標準）}]
  これはグレーのカラーボックスの内容です．
  \tcblower
  これもグレーのカラーボックスの内容です．
\end{tcolorbox}
...
\begin{redbox}[title={赤のカラーボックス}]
  これは赤のカラーボックスの内容です．
  \tcblower
  これも赤のカラーボックスの内容です．
\end{redbox}

\begin{greenbox}[title={緑のカラーボックス}]
  これは緑のカラーボックスの内容です．
  \tcblower
  これも緑のカラーボックスの内容です．
\end{greenbox}

\section{参考文献リストの作成と出力}
\label{sec:bibliography}

\subsection{参考文献の記載時の一般的な注意事項}
\label{ssec:bib-caution}

第~\ref{ssec:bib-caution}~節はこのテンプレートに限らない，一般的に学術論文等で参考文献を載せる際に注意すべき点をまとめています．
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item 著者 3 名以上：
    \end{itemize}
    \item et al. は，ラテン語で「そしてその他（の人々）」を意味する et alii（男性・混合複数），et aliae（女性複数），et alia（中性複数）に由来する略語．\textit{Italic} 体で \textit{et al.} と表記することも多い．
